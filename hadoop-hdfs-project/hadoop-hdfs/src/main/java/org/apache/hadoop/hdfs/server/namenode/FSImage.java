...
public class FSImage implements Closeable {
    ...
    protected FSImage(Configuration conf,
        ...
        //todo     // 设置为true可使NameNode尝试恢复以前失败的dfs.NameNode.name.dir。
        //     // 启用后，将在检查点期间尝试恢复任何失败的目录。
        if (conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_NAME_DIR_RESTORE_KEY,
                DFSConfigKeys.DFS_NAMENODE_NAME_DIR_RESTORE_DEFAULT)) {
            storage.setRestoreFailedStorage(true);
        }
        //todo editsdirs ==> file:/tools/hadoop-3.2.1/data/namenode
        //todo 构建fseditlog
     ...
     * todo saveFSImage()方法会使用一个FSImageFormat.Saver对象来完成保存操作，
     *  FSImageFormat.Saver类会以fsimage文件定义的格式保存Namenode的命名空间信息，
     *  需要注意命名空空间信息会先写入fsimage.ckpt文件中。 saveFSImage()方法还会生成fsimage文件的md5校验文件，
     *  以确保fsimage文件的正确性。
     */
     ...
     * todo * Namenode可以定义多个存储路径来保存fsimage文件，
     *    *
    ...
    private synchronized void saveFSImageInAllDirs(FSNamesystem source,
        ...
        try {
            ...
            //todo       //在每一个保存路径上启动一个线程， 该线程使用FSImageSaver类保存fsimage文件
            for (Iterator<StorageDirectory> it
                 = storage.dirIterator(NameNodeDirType.IMAGE); it.hasNext(); ) {
                StorageDirectory sd = it.next();
                //todo  命名空间具体的保存操作是由FSImageSaver这个类来承担的，
            ...
            //todo       // 将fsimage.ckpt 改名为 fsimage
            ...
            if (!exitAfterSave.get()) {
                //todo //我们已经完成了fsimage的保存， 那么可以将存储上的一部分editlog和fsimage删除
                //        //如果没有成功,则失败.
                purgeOldStorage(nnf);
