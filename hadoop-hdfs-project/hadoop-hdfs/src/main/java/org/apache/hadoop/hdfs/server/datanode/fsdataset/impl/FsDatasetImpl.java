...
class FsDatasetImpl implements FsDatasetSpi<FsVolumeImpl> {
    ...
    //todo   // 当前数据节点的Datanode对象的引用。
    final DataNode datanode;
    private final DataNodeMetrics dataNodeMetrics;
    //todo   // DataStorage对象的引用， 在FsDatasetImpl中调用这个对象开启和关闭存储空间回收站功能。
    final DataStorage dataStorage;
    //todo // FsVolumeList对象的引用， FsDatasetImpl使用FsVolumeList对象提供的功能管理Datanode上的所有数据块，
    //  // 并且通过对volumes字段的修改添加和删除新的存储目录。
    private final FsVolumeList volumes;
    //todo // 在FsDatasetImpl中， 对于每一个存储目录都会构造一个DataStorage对象， storageMap字段就是用来保存这些存储的， 是storageUuid->DataStorage的映射。
    //  // FsDatasetImpl可以通过storageUuid获取对应的DataStorage对象。
    final Map<String, DatanodeStorage> storageMap;
    //todo   // FsDatasetCache类型， 用于将数据块缓存到内存中的工具类。
    ...
    //todo   // ReplicaMap类型， 用于记录Datanode上所有数据块副本的信息。
    final ReplicaMap volumeMap;
    final Map<String, Set<Long>> deletingBlock;
    final RamDiskReplicaTracker ramDiskReplicaTracker;
    //todo // FsDatasetAsyncDiskService类型， 用于在Datanode磁盘存储上进行一些异步的IO操作。
        ...
        try (AutoCloseableLock lock = lockManager.writeLock(LockLevel.VOLUME, bpid,
            ...
            if (!diskFileExists) {
                ...
                if (memBlockInfo == null) {
                    ...
                    //todo           // block文件不存在, 内存中不存在, 但是有原信息文件存在,则删除.
                ...
                if (!memBlockInfo.blockDataExists()) {
                    ...
                    //todo           // block在内存中,但是不在磁盘中, 从内存中清理到元信息.
                    volumeMap.remove(bpid, blockId);
            ...
            //todo       /* block文件存在于disk
            if (memBlockInfo == null) {
                //todo         // 内存中的block信息丢失, 向内存中添加block信息.
                // 构造内存对象
