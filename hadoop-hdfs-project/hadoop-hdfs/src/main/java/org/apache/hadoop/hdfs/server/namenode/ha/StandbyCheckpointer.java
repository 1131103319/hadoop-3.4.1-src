...
public class StandbyCheckpointer {
    ...
    //todo * 整个检查点执行操作的逻辑都是在doCheckpoint()方法中实现的。
    //   *
    ...
    private void doCheckpoint() throws InterruptedException, IOException {
        ...
        try {
            ...
            //todo       //获取当前Standby Namenode上保存的最新的fsimage对象
            FSImage img = namesystem.getFSImage();
            //todo       //获取fsimage中保存的txid， 也就是上一次进行检查点操作时保存的txid
            long prevCheckpointTxId = img.getStorage().getMostRecentCheckpointTxId();
            //todo       //获取当前命名空间的最新的txid， 也就是收到的editlog的最新的txid
            long thisCheckpointTxId = img.getCorrectLastAppliedOrWrittenTxId();
            assert thisCheckpointTxId >= prevCheckpointTxId;
            //todo       //如果相等则没有必要执行检查点操作， 当前fsimage已经是最新的了
            ...
            if (namesystem.isRollingUpgrade()
                ...
                //todo         //如果当前Namenode正在执行升级操作， 则创建fsimage_rollback文件
                imageType = NameNodeFile.IMAGE_ROLLBACK;
            } else {
                //todo         //在正常情况下创建fsimage文件
                imageType = NameNodeFile.IMAGE;
            }
            //todo       //调用saveNamespace()将当前命名空间保存到新的文件中
        ...
        // See HDFS-4816
        //todo                     //todo     //构造一个线程， 通过HTTP将fsimage上传到Active Namenode中
        ...
        private void doWork() {
            ...
            while (shouldRun) {
                ...
                try {
                    ...
                    //todo           //获得最后一次往JournalNode写入的txid和最近一次做检查点的txid的差值
                    final long uncheckpointed = countUncheckpointedTxns();
                    //todo           //计算当前时间和上一次检查点操作时间的间隔
                    ...
                    //todo           //满足检查点执行条件， 则调用doCheckpoint()方法执行检查点操作
                    if (needCheckpoint) {
                        synchronized (cancelLock) {
