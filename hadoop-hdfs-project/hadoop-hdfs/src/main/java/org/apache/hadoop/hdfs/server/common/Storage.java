...
public abstract class Storage extends StorageInfo {
    ...
    public enum StorageState {
        //todo 存储不存在 ■ NON_EXISTENT： 以非FORMAT选项启动时，目录不存在;或者目录不可写、路径为文件时， 存储目录状态都为NOT_EXISTENT状态。
        NON_EXISTENT,
        //todo 未格式化 ■ NOT_FORMATTED： 以FORMAT选项启动时， 都为NOT_FORMATTED状态。
        NOT_FORMATTED,
        //todo 升级完成状态 ■ COMPLETE_UPGRADE： 存在current/VERSION文件， 存在previous.tmp文件夹，则存储目录为升级完成状态。
        COMPLETE_UPGRADE,
        //todo 恢复升级状态 ■ RECOVER_UPGRADE： 存在previous.tmp文件夹， 不存在current/VERSION文件， 存储目录应该从升级中恢复。
        RECOVER_UPGRADE,
        //todo 完成升级提交
        COMPLETE_FINALIZE,
        //todo 完成回滚操作 ■ COMPLETE_ROLLBACK： 存在removed.tmp文件夹， 也存在current/VERSION文件， 则存储目录的回滚操作成功完成。
        COMPLETE_ROLLBACK,
        //todo 恢复回滚 ■ RECOVER_ROLLBACK： 存在removed.tmp文件夹， 不存在current/VERSION文件， 存储目录应该从回滚中恢复。
        RECOVER_ROLLBACK,
        //todo 完成检查点操作 ■ COMPLETE_FINALIZE： 存在finalized.tmp文件夹， 存储目录可以继续执行提交操作。
        COMPLETE_CHECKPOINT,
        //todo 恢复检查点操作
        RECOVER_CHECKPOINT,
        //todo 正常状态 ■ NORMAL： 没有tmp中间状态文件夹， 则存储目录为正常状态。
        NORMAL;
    ...
    public static class StorageDirectory implements FormatConfirmable {
         ...
         * todo ■ getCurrentDir()——获取current目录。
         */
         ...
         * todo ■ getVersionFile()——获取current目录下的VERSION文件。
         */
         ...
         * todo ■ getPreviousVersionFile()——获取previous目录下的VERSION文件。
         */
         ...
         *
         * @return the directory path
         * todo ■ getPreviousDir()——获取previous目录。
         */
         ...
         *
         * @return the directory path
         * todo ■ getPreviousTmp()——获取previous.tmp目录。
         */
         ...
         *
         * @return the directory path
         * todo ■ getRemovedTmp()——获取removed.tmp目录。
         */
         ...
         *
         * @return the directory path
         * todo ■ getFinalizedTmp()——获取finalized.tmp文件。
         */
         ...
         *
         * @return the directory path
         * todo ■ getLastCheckpointTmp()——获取lastcheckpoint.tmp文件。
         */
         ...
         *
         * @return the directory path
         * todo ■ getPreviousCheckpoint()——获取previous.checkpoint文件。
         */
        ...
        public StorageState analyzeStorage(StartupOption startOpt, Storage storage,
            ...
            if (location != null &&
                ...
                //todo         // 如果是外部存储   目前，我们假设提供的存储始终为“正常”
            ...
            try { // check that storage exists
                if (!root.exists()) {
                    ...
                    //todo           //如果存储不存在或者             //如果不是格式化 或者 热切换 则状态为不存在
            ...
            //todo       // 如果startOpt为HOTSWAP，则对于空目录返回NOT_FORMATTED，同时还会检查布局版本。
            ...
            if (!(hasPreviousTmp || hasRemovedTmp
                ...
                if (hasCurrent)
                    ...
                    return StorageState.NORMAL;
                if (hasPrevious)
                    //todo           // 存在previous 文件夹 , 报错, 版本文件丢失
                    throw new InconsistentFSStateException(root,
                            "version file in current directory is missing.");
                if (checkCurrentIsEmpty) {
                    //todo           //检查当前文件夹是否为空,如果为空则抛出异常.
                    checkEmptyCurrent();
                }
                //todo         // 返回状态,未被格式化
                return StorageState.NOT_FORMATTED;
            ...
            if ((hasPreviousTmp ? 1 : 0) + (hasRemovedTmp ? 1 : 0)
                ...
                //todo         // 超过1个临时目录, 报错 : too many temporary directories
                throw new InconsistentFSStateException(root,
                        "too many temporary directories.");

            // # of temp dirs == 1 should either recover or complete a transition
            if (hasCheckpointTmp) {
                //todo         // 是否有hasCurrent 文件 ? 是完成检查点操作 : 否  恢复检查点操作
        ...
        FileLock tryLock() throws IOException {
            ...
            try {
                ...
                LOG.error("Failed to acquire lock on {}. If this storage directory is"
                        ...
                        + " are running.", lockF, e);
                //todo         //读取锁文件失败， 则关闭锁文件， 抛出异常
                file.close();
                throw e;
            }
            if (!deletionHookAdded) {
                //todo         //加锁成功， 在虚拟机运行结束后， 删除锁文件
