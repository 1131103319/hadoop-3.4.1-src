...
public class CacheManager {
     ...
     * todo ■ directivesById： 以缓存指令id为key， 保存所有的缓存指令。
     */
    ...
    /**
     * Cache directives
     * todo ■ directivesByPath： 以路径为key， 保存所有的缓存指令。
     */
     ...
     * todo ■ cachePools： 以缓存池名称为key， 保存所有的缓存池。
     */
     ...
     * todo ■ monitor： CacheReplicationMonitor对象， 负责扫描命名空间和活跃的缓存指令，以确定需要缓存或删除缓存的数据块，
     *    并向Datanode分配缓存任务。
     */
    ...
    private void addInternal(CacheDirective directive, CachePool pool) {
        ...
        //todo     //将缓存指令对象加入directivesByld集合中保存
        directivesById.put(directive.getId(), directive);
        //todo     //将缓存指令对象加入directivesByPath集合中保存
        ...
        //todo     //触发CacheReplicationMonitor执行rescan()操作
        setNeedsRescan();
    }
    ...
    public CachePoolInfo addCachePool(CachePoolInfo info)
        ...
        try {
            ...
            //todo       //如果cachePools字段中已经有了信息， 则抛出异常
            if (pool != null) {
                throw new InvalidRequestException("Cache pool " + poolName
                        + " already exists.");
            }
            //todo       //构造CachePool对象， 对于没有设置的参数， 使用默认值补齐
            pool = CachePool.createFromInfoAndDefaults(info);
            //todo       //将新构造的CachePool对象放入cachePools集合中
