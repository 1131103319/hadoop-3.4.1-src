 ...
 * todo EditLogFileInputStream定义了本地文件系统的editlog文件的输入流。
 *  它定义的方法都很简单， 都是返回了EditLogFileInputStream初始化以后的相应字段，
 *  或者调用了FSEditLogOp.Reader对象的readOp()方法从editlog文件中解析出一个FSEditLogOp对象。
 */
