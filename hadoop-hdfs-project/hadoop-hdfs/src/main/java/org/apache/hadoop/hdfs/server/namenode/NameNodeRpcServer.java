...
public class NameNodeRpcServer implements NamenodeProtocols {
    ...
    @Override // ClientProtocol
    //todo 当客户端删除一个HDFS文件时， 客户端会调用RPC接口ClientProtocol.delete()删除HDFS文件或者目录，
    // 并且删除文件拥有的所有数据块， 以及这个数据块在Datanode上的所有副本，
    // 这个请求会在FSNamesystem.delete() -> FSDirDeleteOp.delete() -> FSDirDeleteOp.deleteInternal()方法中响应.
