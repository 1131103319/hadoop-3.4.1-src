...
/**
 * todo INodeFile类中保存了HDFS文件最重要的两个信息： 文件头header字段和文件对应的数据块信息blocks字段。
 *  header字段保存了当前文件有多少个副本， 以及文件数据块的大小（header字段的处理类似于INode中的permission字段，
 *      前4个比特用于保存存储策略，中间12个比特用于保存文件备份系数
 *      [ 1 + 11   (最高的1位来确定该块是副本还是擦除编码, 0 副本, 1擦除编码  . 对于复制块，尾11位存储复制因子。
 *      对于擦除编码块，尾11位存储EC策略ID ) ]， 后48个比特用于保存数据块大小。 使用内部类HeaderFormat处理） ；
