...
public class DataStorage extends Storage {
    ...
    private StorageDirectory loadStorageDirectory(DataNode datanode,
        ...
        try {
            //todo       // 调用analyzeStorage()方法分析当前StorageDirectory的状态
            StorageState curState = sd.analyzeStorage(startOpt, this, true);
            // sd is locked but not opened
            //todo       // 根据curState恢复状态
            switch (curState) {
                //todo       //存储目录状态正常， 不用执行任何操作
                case NORMAL:
                    break;
                    //todo       //对于不存在的情况， 则直接忽略
                case NON_EXISTENT:
                    ...
                    throw new IOException("Storage directory with location " + location
                            + " does not exist");
                    //todo       //没有格式化时， 调用format()方法格式化数据目录
                case NOT_FORMATTED: // format
                    ...
                    break;
                    //todo         //对于其他情况， 则调用StorageDirectory.doRecover()恢复到NORMAL状态
            ...
            //todo       //调用doTransition()执行启动操作， 启动选项通过startOpt参数传递
            if (!doTransition(sd, nsInfo, startOpt, callables, datanode.getConf())) {

                // 3. Update successfully loaded storage.
                //todo         //3． 对于每一个成功执行的存储目录， 写入VERSION文件
    ...
    void recoverTransitionRead(DataNode datanode, NamespaceInfo nsInfo,
        ...
        //todo     // 调用addStorageLocations()方法对Datanode存储空间进行初始化
    ...
    private boolean doTransition(StorageDirectory sd, NamespaceInfo nsInfo,
        ...
        //todo     // 检测 NamespaceID 是否相等 , 不相等 则报错
