...
public class BlockManager implements BlockStatsMXBean {
    ...
    public BlockManager(final Namesystem namesystem, boolean haEnabled,
        ...
        //todo     // 构建createBlockTokenSecretManager
        ...
        //todo 块报告期间要记录信息的最大块数。
        //todo     // dfs.namenode.max-num-blocks-to-log : 1000 [对块报告后由NAMENODE打印到日志块的数量进行限制。]
    ...
    private void printInitialConfigs() {
        /**
         * todo defaultReplication         = 3      默认副本 3
         //todo  maxReplication             = 512    最大副本 512
         //todo  minReplication             = 1      最小副本 1
         //todo  maxReplicationStreams      = 2      最大复制流限制(可能会影响到datanode)
         //todo  redundancyRecheckInterval  = 3000ms 检查频率
         //todo  encryptDataTransfer        = false  是否颁发块加密密钥。
         //todo  maxNumBlocksToLog          = 1000   块报告期间要记录信息的最大块数。
         */
     ...
     * //todo    * 调度datanode  删除block
     */
    int computeInvalidateWork(int nodesToProcess) {
        //todo     //获取要删除的block所在的datanode节点
        final List<DatanodeInfo> nodes = invalidateBlocks.getDatanodes();
        //todo     //打乱排序
        Collections.shuffle(nodes);
        //todo     //计算出需要操作的任务数量 (取 nodes 和 nodesToProcess 中的最小值 )
        nodesToProcess = Math.min(nodes.size(), nodesToProcess);

        int blockCnt = 0;
        for (DatanodeInfo dnInfo : nodes) {
            //todo       // 将要删除的块 加入到 datanode对应的 DatanodeDescriptor#invalidateBlocks 缓存中,
            // 会通过心跳的方式发送给具体的datanode执行删除操作.
     ...
     * @throws IOException
     * todo * processReport()方法会判断当前块汇报是否是该数据节点的第一次块汇报，
        ...
        try {
            ...
            if (!storageInfo.hasReceivedBlockReport()) {
                ...
                blockLog.info("BLOCK* processReport 0x{} with lease ID 0x{}: Processing first "
                        ...
                        nodeID);
                //todo         //  对于第一次块汇报， 调用processFirstBlockReport()
                ...
                if (!StorageType.PROVIDED.equals(storageInfo.getStorageType())) {
                    //todo           //不是第一次块汇报， 则调用私有的processReport()方法
    ...
    /**
     * todo 对于Datanode周期性的块汇报， processReport()方法会调用私有的processReport()方法处理。 这个方法会调用reportDiff()方法， 将块汇报中的副本与当前Namenode内存中记录的副本状态做比对， 然后产生5个操作队列
    ...
    Collection<Block> processReport(
        ...
        //todo     //reportDiffSorted()方法获取需要更新的不同队列
     ...
     * @throws IOException
     * todo * Datanode会通过调用NamenodeProtocol.blockReport()方法向Namenode发送全量块汇
     ...
     * todo * 当Datanode上写入了一个新的数据块副本或者完成了一次数据块副本复制操作后， 会
    ...
    private Block addStoredBlock(final BlockInfo block,
        ...
        //todo     // 如果当前block不属于任何inode， 则直接返回， 不进行任何操作
        ...
        //todo     // 在block -> datanode映射中添加当前datanode
    ...
    private MisReplicationResult processMisReplicatedBlock(BlockInfo block) {
        ...
        if (shouldProcessExtraRedundancy(num, expectedRedundancy)) {
            ...
            //todo //如果此块的任何副本位于被视为“stale”的节点上，则这些副本实际上可能已被删除。
    ...
    /**
     * todo processIncrementalBlockReport()方法会遍历增量汇报中的所有数据块， 如果是新添加的数据块（RECEIVED_BLOCK） ，
     *  则调用addBlock()方法处理添加请求； 如果是删除的数据块（DELETED_BLOCK） ，
     *  则调用removeStorageBlock()修改数据块与存储这个数据块的数据节点存储的对应关系。
     *  对于接收中的副本（RECEIVING_BLOCK） ,则调用processAndHandleReportedBlock()方法处理。
     * @param node
     * @param srdb
     * @throws IOException
     */
     ...
     * todo * 计算 DataNode 节点上 块复制& 块失效的任务
    ...
    int computeDatanodeWork() {
        ...
        // fixed up later.
        //todo     // 在安全模式下, Block 不允许复制或者移除.
        if (namesystem.isInSafeMode()) {
            return 0;
        }
        //todo     // 存活的 datanode 的数量
        final int numlive = heartbeatManager.getLiveDatanodeCount();
        //todo // 可以进行复制的 blocks 的复制因子 , 默认: numlive * 2 ,
        //    // 即 最多复制datanode节点数2倍的block. 为了防止网络阻塞.
        final int blocksToProcess = numlive
                * this.blocksReplWorkMultiplier;
        //todo     // 进行删除时的阈值  numlive * 32%  即, 只有三分之一的datanode 会进行删除操作
        final int nodesToProcess = (int) Math.ceil(numlive
                * this.blocksInvalidateWorkPct);
        //todo     //调用computeReplicationWork()计算出需要进行备份的副本
        ...
        //todo     //调用computeInvalidateWork()计算出需要进行删除的副本
