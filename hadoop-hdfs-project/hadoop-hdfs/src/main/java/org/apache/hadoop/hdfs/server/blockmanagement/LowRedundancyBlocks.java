...
class LowRedundancyBlocks implements Iterable<BlockInfo> {
     ...
     */
    //todo 数据块的副本数量已经足够， 但是数据块副本的分布不是很好， 如果一个机架或者交换机宕机很有可能造成数据块完全丢失。
    static final int QUEUE_REPLICAS_BADLY_DISTRIBUTED = 3;
    /** The queue for corrupt blocks: {@value} */
    //todo 保存已经损坏的数据块， 就是该数据块对应的所有副本都损坏了。
    // 这里的策略是将损坏的数据块放入这个队列中， 对没有损坏的副本给予更高的优先级
    ...
    private int getPriority(BlockInfo block,
        ...
        if (curReplicas >= expectedReplicas) {
            //todo       //数据块的副本数量足够， 但是没有均匀地分布在不同的机架上， 返回优先级3
    ...
    private int getPriorityContiguous(int curReplicas, int readOnlyReplicas,
        ...
        if (curReplicas == 0) {
            ...
            //todo       // //当前数据块没有有效的副本， 并且有些副本所在的Datanode正处于离线中， 优先级 0
