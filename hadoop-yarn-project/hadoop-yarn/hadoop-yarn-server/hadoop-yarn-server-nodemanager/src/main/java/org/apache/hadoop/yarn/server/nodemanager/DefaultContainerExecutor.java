 ...
 */
//todo DefaultContainerExecutor，简称DCE。
// 每个Container运行在单独的进程里，但进程都是由NM的用户启动的。
// 比如NM进程是用yarn用户启动的，那么所有Container的进程也由yarn用户启动。

/**
 * todo 在ContainerExecutor启动一个Container的过程中，涉及到了三个脚本，它们分别是:
 * todo default_container_executor.sh 启动
 * todo default_container_executor_session.sh 启动
 * todo launch_container.sh 负责运行相应的Container，也能是MRAppMaster，也可能是Mapper或者Reducer:
...
public class DefaultContainerExecutor extends ContainerExecutor {
    ...
    public void startLocalizer(LocalizerStartContext ctx)
        ...
        //todo   // create $local.dir/usercache/$user and its immediate parent
