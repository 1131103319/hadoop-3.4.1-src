...
public class CallQueueManager<E extends Schedulable>
    ...
    CallQueueManager(BlockingQueue<E> queue, RpcScheduler scheduler,
        ...
        //todo * 启用Backoff配置参数。
        //todo   * 当前，如果应用程序中包含较多的用户调用，假设没有达到操作系统的连接限制，则RPC请求将处于阻塞状态。
        //todo   * 或者，当RPC或NameNode在重负载时，可以基于某些策略将一些明确定义的异常抛回给客户端，
        //todo   * 客户端将理解这种异常并进行指数回退，
        //todo   * 以此作为类RetryInvocationHandler的另一个实现
        this.clientBackOffEnabled = clientBackOffEnabled;
